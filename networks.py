...
class Net(nn.Module):
    def __init__(self):
        ...
        self.hidden_size = HIDDEN_SIZE_NET
...
class ControlNet(nn.Module):
    def __init__(self, a=1.2):
        ...
        self.input_size = (
            INPUT_SIZE + HIDDEN_SIZE_NET + OUTPUT_SIZE
        )  # 8 input + 20 hidden + 2 output
        self.hidden_size = HIDDEN_SIZE_CTRL
        self.output_size = (
            HIDDEN_SIZE_NET + OUTPUT_SIZE
        )  # Control signals for hidden + output neurons
        self.layer1 = nn.Linear(self.input_size, self.hidden_size)
        self.layer2 = nn.Linear(self.hidden_size, self.output_size)
        self.relu1 = nn.ReLU()
        self.relu2 = nn.ReLU()
